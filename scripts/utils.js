...
function requireCookieStore() {
    if (!("cookieStore" in window)) throw new Error("Откройте сайт через localhost или HTTPS в браузере с поддержкой Cookie Store API.");
...
function studentRecords(cookies) {
    ...
    for (const cookie of cookies.filter(item => item.name.startsWith("student_"))) {
        let student;
        try { student = JSON.parse(decodeURIComponent(cookie.value)); }
        catch { throw new Error("Не удалось прочитать сохранённую запись " + cookie.name); }
...
async function changeStudentCookies(operations, snapshot) {
    ...
    try {
        for (const operation of operations) {
            ...
            if (operation.value === null) await storage.delete({ name: operation.name, path: "/" });
            else await storage.set({ ...operation, path: "/", expires: Date.now() + 86400000, sameSite: "lax" });
            const saved = await storage.get(operation.name);
            if (operation.value === null ? saved != null : saved?.value !== operation.value) throw new Error("Браузер не подтвердил сохранение данных.");
        }
    } catch (error) {
        let restored = true;
        for (const name of changed.reverse()) {
            ...
            try {
                if (previous) await storage.set({ name, value: previous.value, path: "/", expires: previous.expires ?? Date.now() + 86400000, sameSite: "lax" });
                else await storage.delete({ name, path: "/" });
...
async function saveStudent(student, originalIsu = null) {
    ...
    const operations = [{ name: key, value }, ...legacyCookies(snapshot, originalIsu ?? student.isu).map(cookie => ({ name: cookie.name, value: null }))];
